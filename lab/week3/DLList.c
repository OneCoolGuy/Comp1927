...
{
	/* So the node before current has to point to the new node, the current node has to point backwards to the 
	 * new node and the new node has to point to them as well, and remenber to change the current to be
	 * the new node.
	 */
	DLListNode *new;
	assert(L != NULL); 
